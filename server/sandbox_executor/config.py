# Resource limits and security settings for sandbox execution

# Docker resource limits
CPU_SHARES = 512  # Half CPU
MEMORY = "128m"  # 128 MB
MEMORY_SWAP = "256m"  # 256 MB swap
CPU_PERIOD = 100000
...
# Security settings
SECCOMP_PROFILE = ""  # Use default seccomp profile, or path to custom
READONLY_ROOTFS = True  # Read-only root filesystem
NO_NEW_PRIVILEGES = True  # No new privileges

# Ephemeral workspace
WORKSPACE_MOUNT = "/tmp/workspace"  # Host path for workspace, but since ephemeral, use tmp

# Artifact capture
ARTIFACT_DIR = "/artifacts"  # Directory inside container to capture

# Base image for execution
BASE_IMAGE = "alpine:latest"  # Lightweight base image

# Allowed syscalls for seccomp (if custom profile)
ALLOWED_SYSCALLS = [
    ...
    "uname", "semget", "semop", "semctl", "shmdt", "msgget", "msgsnd",
    "msgrcv", "msgctl", "fcntl", "flock", "fsync", "fdatasync", "truncate",
    ...
    "getrusage", "sysinfo", "times", "ptrace", "getuid", "geteuid", "getgid",
    "getegid", "getgroups", "setgroups", "setuid", "setgid", "seteuid", "setegid",
    "getresuid", "getresgid", "setresuid", "setresgid", "getpgid", "setpgid",
    "getsid", "setsid", "getpriority", "setpriority", "sched_setparam",

...
from providers.together_adapter import TogetherAdapter

# Load environment variables from .env file
load_dotenv()

def test_together_tools():
    """Test if Together supports tools"""
    ...
    print("Testing Together tools support...")
    print(f"Tools: {json.dumps(tools, indent=2)}")

    try:
        adapter = TogetherAdapter(role="builders")

        response = adapter.call_model(messages, temperature=0.7, tools=tools)
        print("✓ SUCCESS - Together supports tools!")
        print(f"Response: {response.text[:200]}...")
        print(f"Tool calls: {response.tool_calls}")

    except Exception as e:
        print(f"✗ FAILED - Together tools error: {str(e)}")

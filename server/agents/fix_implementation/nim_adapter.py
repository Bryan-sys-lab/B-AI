...
class NIMAdapter(BaseAdapter):
    def __init__(self, role: str = "default"):
        if role == "thinkers":
            model = "nvidia/llama-3.1-70b-instruct"  # Advanced reasoning model
        elif role == "gateways":
            model = "nvidia/llama-3.1-51b-instruct"  # Efficient for routing
        else:
            model = "nvidia/llama-3.1-51b-instruct"  # Default
        # Pick model via registry, then initialize and fall back to legacy env var
